...
<Playground>
{
  ()=>{
    function Demo(){
      const { list, merge, insert, replace, remove, getKey, unshift, shift, pop, move } = useDynamicList([1,2,3]);
...
<Playground>
{
  ()=>{
    const Demo = Form.create()((props) => {
      const { list, merge, insert, replace, remove, getKey, move, push, unshift, pop } = useDynamicList([{name: 'my bro', age: '23', memo: 'he\'s my bro' }, {}, {}])
