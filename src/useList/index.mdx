...
<Playground>
{
  ()=>{
    const Demo = Form.create()((props) => {
      const { list, merge, insert, replace, remove, getKey, move, push, unshift, pop } = useList([{name: 'my bro', age: '23', memo: 'he\'s my bro' }, 0, 0])
      ...
      return (
        <>
          <Table columns={columns} dataSource={list} rowKey={(r, index)=>getKey(index)} pagination={false} />
          <Button onClick={()=>push(0)}>Add one line</Button>
