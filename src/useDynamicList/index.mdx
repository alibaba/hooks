    ...
    const Card = (props) => {
      ...
      return (
        <div style={{ border: '1px solid #e8e8e8', padding: 16, marginBottom: 16 }}>
          <Form.Item label='组名称'>
            <Input placeholder='请填写组名称' defaultValue={props.name} />
          </Form.Item>
          ...
          <Form.Item label='投放节奏'>
          {
            list.map((ele, index)=>(
              <div style={{marginBottom: 16}} key={getKey(index)}>
                <Input placeholder='请填写广告名称' defaultValue={ele.name} addonBefore={'名称：'} />
                <Input placeholder='请填写投放节奏' defaultValue={ele.value} addonAfter={'次/天'} />
              </div>
            ))
          }
          </Form.Item>
    ...
    const Demo = (props) => {
      ...
      return (
        <div style={{width: 400, margin: 'auto'}}>
          {list.map((ele, index)=><Card key={getKey(index)} list={ele.list} name={ele.name} />)}
