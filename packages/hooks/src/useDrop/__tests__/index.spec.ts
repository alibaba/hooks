...
describe("useDrop", () => {
  ...
  test("should add/remove listener on mount/unmount", () => {
    ...
    expect(mockTarget.addEventListener).toHaveBeenCalledTimes(
      eventNames.length
    );
    ...
    unmount();
    expect(mockTarget.removeEventListener).toHaveBeenCalledTimes(
      eventNames.length
    );
    eventNames.forEach((eventName, i) => {
  ...
  test("should call onUri on drop", async () => {
    ...
    vi.spyOn(mockEvent.dataTransfer, "getData").mockImplementation(
      (format?: string) => {
